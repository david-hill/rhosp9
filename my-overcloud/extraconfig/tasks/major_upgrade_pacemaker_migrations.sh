...
function liberty_to_mitaka_keystone {
    ...
    if [[ -n $(is_bootstrap_node) ]]; then
        ...
        # Make sure we support all pcs output version for constraint.
        if [ -z "$($PCS_CONFIG | grep keystone | grep start | grep then | head -1 | awk '{print $7}' )" ]; then
            PCS_CONFIG="${PCS} config --full"
        fi

        # change all constraints from keystone to dummy
        CONSTR="$($PCS_CONFIG | grep keystone | grep start | grep then)"
